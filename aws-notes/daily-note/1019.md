...
ステートレス（stateless） = サーバーが「セッションや状態（state）」を保持しない。
→ 各リクエストは独立しており、どのサーバーに送っても同じ結果になる。

ステートフル（stateful） = サーバーが何らかの状態やデータを保持している。
...
ALB は VPC（Virtual Private Cloud）内に作成されるロードバランサー です。

その中で、複数のサブネット（＝AZ単位）にまたがって配置されます。

つまり、「VPCの外にある」わけではありません。
...
ALBを作成する際、2つ以上のサブネット（通常は異なるAZのもの）を指定します。

これにより、ALBは各サブネットに ENI（Elastic Network Interface）＝ネットワークインターフェイス を作成し、各AZに「入り口」を持ちます。

その結果、AZ障害が発生しても他のAZのALBエンドポイントが生き残り、可用性が確保されます。

→ つまり「ALBは複数AZにまたがるが、VPCの中にある」が正確な表現です。
...
ALB（Application Load Balancer）は、ターゲットグループ単位でヘルスチェックのパスを自由に指定できます。
...
ALB自体にはヘルスチェック機能がありますが、
実際の設定は ターゲットグループごと に行います。
...
CloudFormationを中に使っているが、
そのテンプレートをGUIで「安全に配布・実行」できる仕組み。

管理者が「Product（テンプレート）」を用意し、
ユーザーは「Launch」ボタンを押すだけで構築可能。
...
Service Catalogは「CloudFormationを安全に呼び出す仕組み」であり、
実際にリソースを作っているのはCloudFormationです。
...
指定するロールを「CloudFormation 実行ロール（Execution Role）」と呼びます。
...
CloudFormationでは、
「スタックを作る人の権限」と「リソースを実際に作る権限」を分けられます。
