...
1. グローバルユーザー向けの低レイテンシアクセス

固定のAnycast IP を世界中のユーザーに提供

ユーザーは常に同じIPに接続 → AWSグローバルネットワークに最も近いエッジロケーションから入口

そこから AWS バックボーンを経由して最適なリージョンにルーティング

例：

東京のユーザー → 東京リージョンへ

ニューヨークのユーザー → バージニアリージョンへ

> **最大の売り**: 「世界中のユーザーを最寄りリージョンに自動誘導する」

### 2. 高可用性と即時フェイルオーバー

- **Route 53 の DNS切替**はキャッシュがあり、数十秒〜数分のラグあり
- **Global Accelerator は DNSを介さずネットワークレベルでルーティング** → 切替はほぼリアルタイム

#### 例
- **東京リージョンがダウンしたら、自動でシンガポールリージョンへ転送**
- **ユーザーは同じIPにアクセスしているだけなので意識しなくてよい**

> **多い使い方**: 「DR切替をシームレスにする」

### 3. セキュリティとシンプルさ

- **アプリケーションに固定のグローバルIP を持たせられる**
- **Route 53 は名前解決依存なのでIPは動的**
- **セキュリティポリシーやファイアウォール設定で「固定IPが必須」なケースに便利**

#### 例

- **ゲームや金融系アプリで「許可する接続元IPを固定」したい**
- **APIクライアントに「必ずこのIPを使ってね」と明示できる**

## Global Accelerator のメイン用途まとめ

**Global Accelerator のメイン用途は3つ**
