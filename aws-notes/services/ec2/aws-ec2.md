...
> Point: AMIがあれば同じ構成のインスタンスを何台でも複製して起動できる仕組み
...
AMI + インスタンスタイプ + ネットワーク設定 + キーペア などをまとめて保存した設計図
...
- Auto Scaling Group (ASG) や EC2 Fleet のベース設定に使う
- 設定をバージョン管理できる（v1, v2, … と作れる）
...
#### 公開AMI
Amazon公式AMI（Amazon Linuxなど）は public 設定なので誰でも使える
...
EC2 は CloudWatch にメトリクスを自動で送信するが、2つのモードがある

#### 基本モニタリング (Basic Monitoring)
- デフォルト
- 5分間隔でメトリクス（CPU利用率、ネットワーク、ディスクなど）が収集される
...
## 詳細モニタリング vs CloudWatch エージェント

### 詳細モニタリング (Detailed Monitoring)
- 対象: EC2
- できること: 収集するメトリクスの「時間粒度」を 5分 → 1分 にするだけ
- 情報の種類は増えない（CPU、ネットワーク、ディスクなど基本メトリクスのみ）
...
### 使い分け
- 「もっと細かく見たい」→ 詳細モニタリング
- 「もっと多くの種類を見たい」→ CloudWatch エージェント

---

## EC2 の休止 (Hibernate) 状態

### 特徴
- メモリ内容を EBS に保存してからインスタンスを停止
- 再開時にメモリ内容を復元するので、アプリやプロセスを起動し直さずにすぐ使える
- 状態的には stopped に似ているけど「メモリ内容を保持している」点が違う

### 休止が使える条件
- インスタンスタイプがサポート対象（多くの汎用/メモリ最適化タイプなど）
- EBS バックドインスタンス（インスタンスストアは不可）
- ルートボリュームが 暗号化されていること
- メモリサイズと同等かそれ以上のルートボリューム容量が必要（メモリ内容を保存するため）

### ステータス的な見え方
- コンソールや CLI 上では 「stopped」状態 として表示される
- ただし 停止方法が Hibernate の場合は、内部的に「休止状態」として扱われる

> つまり「休止中」という独立したステータスはなく、UI上は stopped に見えるけど「Hibernate で停止した」とメタデータで区別されるイメージ

### まとめ
- EC2 に休止モードはある
- ただしステータスとしては「stopped」に見える
- Hibernate で停止すると、メモリ内容を保持したまま復元できる