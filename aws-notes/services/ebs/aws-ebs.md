...
EC2 を再起動すれば元のEBSをそのまま使える

 「停止 = 電源OFF」イメージ
→ データは消えない

2. EC2 を「終了 (Terminate)」した場合

ルートボリューム（OSが入ってるEBS）は デフォルトで削除される（DeleteOnTermination=Yes）

追加でアタッチしたデータボリュームは 削除されない

 「終了」時に消えるのは「OS用のEBS（ルート）」だけがデフォルト動作
 オプションで「保持する」に変えられる
...
デフォルトでは DeleteOnTermination = true → インスタンス終了時に削除される

追加のデータボリューム

デフォルトでは DeleteOnTermination = false → インスタンス終了しても残る

Amazon EBS ストレージタイプと IOPS 特性
1. 汎用 SSD（gp2 / gp3）

gp2

IOPS: 3 IOPS/GB（最大 16,000 IOPS）

ボリュームサイズに比例して性能が決まる

バースト性能あり（3000 IOPSまでバースト可能）

gp3

IOPS: 最大 16,000 IOPS を独立して指定可能

スループット: 最大 1,000 MB/s

容量と性能が切り離されている（柔軟に設定可能）

2. プロビジョンド IOPS SSD（io1 / io2）

io1 / io2

IOPS: 最大 64,000 IOPS（Nitro世代インスタンス利用時）

1ボリュームあたり 50 IOPS/GB まで設定可能

高耐久が必要なワークロード（io2 のほうが耐久性高い）

io2 Block Express

IOPS: 最大 256,000 IOPS

スループット: 最大 4,000 MB/s

超大規模データベース（SAP HANA, Oracle, SQL Server など）向け
...
バースト時の IOPS: 数千程度（最大 500 IOPS/GB くらいのイメージ）

大量データのシーケンシャル処理（ビッグデータ、ログ処理）向け
...
IOPS: 数百程度（最大 250 IOPS/GB くらいのイメージ）

コールドデータ、アクセス頻度の低いワークロード向け
...
→ このときに Cluster Placement Group と組み合わせると、低レイテンシかつ高スループットで効率的に動く